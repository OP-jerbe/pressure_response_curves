...
def load_data(file_pattern):
    ...
    concatenated_df = pd.concat(df_list, ignore_index=True)  # Concatenate all DataFrames
    concatenated_df['Time'] = pd.to_datetime(concatenated_df['Time'], format='%m/%d/%Y %I:%M:%S %p')  # Convert 'Time' column to datetime
...
def plot_multi_time_windows(data, time_windows, y_column, invert_y):
    ...
    for window in time_windows:
        ...
        # Plot the data
        plt.scatter(filtered_data['Source Pressure (mBar)'], filtered_data[y_column],
...
def plot_button_click():
    ...
    for i in range(len(start_times)):
        try:
            ...
            messagebox.showerror("Datetime Error", "Incorrect datetime format. Please use YYYY-MM-DD HH:MM:SS")
            return
...
def export_button_click():
    with filedialog.asksaveasfile(mode='w', defaultextension=".txt") as file:
        if file:
            ...
            for i in range(len(start_times)):
                file.write(f"{start_times[i].get()}\t{end_times[i].get()}\t{legend_entries[i].get()}\n")
...
def add_time_window():
    ...
    start_time_entry.grid(row=len(start_times) + 1, column=0, padx=(10, 5), pady=5, sticky='w')
    ...
    end_time_entry.grid(row=len(end_times) + 1, column=1, padx=(10, 5), pady=5, sticky='w')
    ...
    legend_entry.grid(row=len(legend_entries) + 1, column=2, padx=(10,5), pady=5, sticky='w')
...
def update_y_variable_options(directory):
    ...
    if files:
        ...
        if "Source Pressure (mBar)" in columns:
            columns.remove("Source Pressure (mBar)")  # Remove "Source Pressure (mBar)" header
...
datetime_example_label = ttk.Label(datetime_example_frame, text="Datetime Format: YYYY-MM-DD HH:MM:SS", anchor='w')
...
invert_y_checkbutton = ttk.Checkbutton(y_variable_frame, text="Invert Y-Axis", variable=invert_y_var)
...
add_button = ttk.Button(add_remove_frame, text="Add Time Window", command=add_time_window)
add_button.grid(row=0, column=0, padx=5, pady=5, sticky='ew')

# Button to remove time windows
remove_button = ttk.Button(add_remove_frame, text="Remove Time Window", command=remove_time_window)
